...
将 EPUB 文件解析为 Markdown 文本和可选的嵌入式图片。
"""
...
import ebooklib  # type: ignore[import-untyped]  # ebooklib 无 stub（无 py.typed），模块整体按 Any 处理
...
class EPUBParser(BaseParser):
    ...
    def _parse_epub_fallback(self, content: bytes) -> ParsedDocument:
        """当 ebooklib 无法读取时，直接以 ZIP 方式解析 EPUB 文件。"""
        import re
        import zipfile
                ...
                for img_file in epub_zip.namelist():
                    if img_file.lower().endswith(img_exts):
                        try:
                            ...
                            img_path = f"images/{uuid7().hex}{ext}"
    ...
    def _extract_metadata(self, book: Any) -> Dict[str, Any]:
        # 返回值后续还会写入 file_size / chapter_count 等 int 字段，故为 Dict[str, Any]
    ...
    def _extract_content(self, book: Any) -> tuple[str, Dict[str, str]]:
    ...
    def _process_chapter(
        self,
        html_item: Any,  # ebooklib 无 stub，item 类型按 Any
        toc_index: int = 0,
    ...
    def _strip_internal_links(soup: BeautifulSoup) -> None:
        ...
        EPUB 内部链接（其他章节文件、``#fragment`` 锚点、目录条目）在提取后
        会成为死链接。仅保留外部链接，其余替换为纯文本。
        """
